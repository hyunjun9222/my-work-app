...
지표(이수율·탈락률)는 여기서 다시 계산하지 않는다. 주차 파일을 행 목록으로
...
for 이름, 이번키, 작년키 in 구간:
    ...
    행 = {
        ...
        "훈련목표인원": t["훈련목표인원"] if t else 0,
        "훈련수료인원": t["훈련수료인원"] if t else 0,
        "중도탈락자": t["중도탈락자"] if t else 0,
        "이수율": num(t["이수율"]) if t else None,
        "탈락률": num(t["탈락률"]) if t else None,
        "제외": t["제외"] if t else 0,
        "_이수율원문": t["이수율"] if t else None,
    }
    if 비교:
        행["작년 이수율"] = num(p["이수율"]) if p else None
        행["작년 수료인원"] = p["훈련수료인원"] if p else 0
        행["증감(%p)"] = (
            round((행["이수율"] - 행["작년 이수율"]) * 100, 1)
            if 행["이수율"] is not None and 행["작년 이수율"] is not None
            else None
        )
    표.append(행)
...
if 있는구간.empty:
    ...
    총목표 = int(있는구간["훈련목표인원"].sum())
    총수료 = int(있는구간["훈련수료인원"].sum())
    총과정 = int(있는구간["집계 과정수"].sum())
    총제외 = int(있는구간["제외"].sum())
    전체이수율 = 총수료 / 총목표 if 총목표 else None
    최고 = 있는구간.loc[있는구간["이수율"].idxmax()] if 있는구간["이수율"].notna().any() else None
    최저 = 있는구간.loc[있는구간["이수율"].idxmin()] if 있는구간["이수율"].notna().any() else None

    문장 = [
        f"**{단위}** 기준으로 {len(있는구간)}개 구간, 과정 {총과정:,}개를 집계했습니다."
        + (f" ({걸러냄})" if 걸러냄 else ""),
        f"훈련목표 {총목표:,}명 대비 수료 {총수료:,}명 — 전체 이수율 **{pct_text(전체이수율)}** 입니다.",
    ]
    if 최고 is not None and 최저 is not None and len(있는구간) > 1:
        문장.append(
            f"가장 높은 구간은 {최고['구간']}({최고['이수율'] * 100:.1f}%), "
            f"가장 낮은 구간은 {최저['구간']}({최저['이수율'] * 100:.1f}%) 입니다."
        )
    if 비교 and "증감(%p)" in 있는구간 and 있는구간["증감(%p)"].notna().any():
        ...
        문장.append(
            ...
            f"이수율이 오른 곳 {오른곳}개 · 내린 곳 {내린곳}개입니다."
        )
    elif 비교:
...
st.dataframe(
    ...
    width="stretch",
    column_config={
        "이수율": st.column_config.NumberColumn("이수율", format="percent"),
        "탈락률": st.column_config.NumberColumn("탈락률", format="percent"),
        "작년 이수율": st.column_config.NumberColumn("작년 이수율", format="percent"),
        "증감(%p)": st.column_config.NumberColumn("증감(%p)", format="%.1f"),
    },
)
if (df["_이수율원문"] == UNVERIFIED).any() or (df["_이수율원문"] == UNCOMPUTABLE).any():
    st.caption("일부 구간은 입력 오류(검증 필요)나 분모 0(계산 불가)이라 이수율이 비어 있습니다.")

# ── 세로 막대 그래프
st.subheader("세로 막대 그래프")
탭1, 탭2 = st.tabs(["이수율", "수료인원"])

with 탭1:
    칼럼 = ["이수율"] + (["작년 이수율"] if 비교 and df["작년 이수율"].notna().any() else [])
    그래프 = df.set_index("구간")[칼럼].mul(100).round(1)
    st.bar_chart(그래프, height=380, y_label="이수율 (%)", x_label="구간")
    if len(칼럼) == 1 and 비교:
...
with st.expander("이 화면이 쓰는 자료"):
    ...
    st.write("- 이수율 = 훈련수료인원 ÷ 훈련목표인원, 탈락률 = 중도탈락자 ÷ 훈련실시인원 (집계기에서 계산)")
